...
export class PatientViewDetailsService {
  ...
  getJoinedQueueStatus(req?: any){
    ...
    return this.http.post('https://c3cwmli2ne.execute-api.us-east-1.amazonaws.com/equeue/api/get-joined-queue-status', req, { headers: this.httpOptions, observe: 'response' }).pipe(map(
      response => {
        if (response.status === 200) {
          console.log('response is 200');
